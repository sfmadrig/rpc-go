...
func TestWiredCmd_Validate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cmd:     &WiredCmd{ConfigureBaseCmd: ConfigureBaseCmd{}, IPAddress: "192.168.1.100"}, // Missing subnet mask, gateway, DNS
			wantErr: true,
		},
		{
			name:    "invalid - bad IP format",
			cmd:     &WiredCmd{ConfigureBaseCmd: ConfigureBaseCmd{}, IPAddress: "invalid-ip", SubnetMask: "255.255.255.0", Gateway: "192.168.1.1", PrimaryDNS: "8.8.8.8"},
			wantErr: true,
		},
		{
		...
		},
		{
			...
			cmd:     &WiredCmd{ConfigureBaseCmd: ConfigureBaseCmd{AMTBaseCmd: commands.AMTBaseCmd{ControlMode: 1}}, IEEE8021xProfileName: "test8021x", IEEE8021xUsername: "testuser", IEEE8021xAuthenticationProtocol: 0, IEEE8021xPrivateKey: "test-private-key", IEEE8021xClientCert: "test-client-cert", IEEE8021xCACert: "test-ca-cert", IPAddress: "192.168.1.100", SubnetMask: "255.255.255.0", Gateway: "192.168.1.1", PrimaryDNS: "8.8.8.8"},
			wantErr: false,
		},
	}
...
func TestWiredCmd_Run(t *testing.T) {
	...
	t.Run("structure_validation", func(t *testing.T) {
		cmd := &WiredCmd{ConfigureBaseCmd: ConfigureBaseCmd{}, IPAddress: "192.168.1.100", SubnetMask: "255.255.255.0", Gateway: "192.168.1.1", PrimaryDNS: "8.8.8.8", SecondaryDNS: "8.8.4.4"}

		// Verify command has required fields (password now provided via global context, not struct)
